...
#include "OnlineRPGGameMode.generated.h"

class AOnlineRPGAIController;
class AOnlineRPGPlayerState;
class AOnlineRPGPickup;
class APlayerStart;
class FUniqueNetId;

UCLASS(config=Game)
class ONLINERPG_API AOnlineRPGGameMode : public AGameMode
{
	...
	virtual bool CanDealDamage(AOnlineRPGPlayerState* DamageInstigator, AOnlineRPGPlayerState* DamagedPlayer) const;
	...
	/** Create a bot */
	AOnlineRPGAIController* CreateBot(int32 BotNum);	
...
protected:
	...
	// UPROPERTY()
	// TArray<AOnlineRPGAIController*> BotControllers;

	// UPROPERTY(config)
	// TSubclassOf<class AOnlineRPGPlayerController> PlatformPlayerControllerClass;
	...
	virtual void InitBot(AOnlineRPGAIController* AIC, int32 BotNum);

	/** check who won */
	virtual void DetermineMatchWinner();

	/** check if PlayerState is a winner */
	virtual bool IsWinner(AOnlineRPGPlayerState* PlayerState) const;
...
public:	
	...
	// UPROPERTY()
	// TArray<AOnlineRPGPickup*> LevelPickups;
